...
# Launch Readiness v3: CloudWatch AI Control Plane
...
**v3 changes from v2:** Updated to align with PRD v2 (Solution Lineage, enhanced personas, falsifiable failure scenarios) and design-spec-v3 (Cost Intelligence page, cost recommendation Alert, confidence badges). Key additions: (1) Cost Intelligence page engineering requirements, (2) discovery confidence scoring subsystem, (3) updated acceptance criteria for v3 interactions, (4) new tech debt items for v3 features, (5) updated success metrics with cost optimization targets.
...
- **v3: Model comparison engine (PRD v2 Capability 4):** Quality-aware recommendations that compare current model against cheaper alternatives. Uses Bedrock benchmark data (accuracy by task type) to estimate accuracy equivalence. Recommendation confidence threshold: >80% accuracy equivalence required to surface. Output: current model, suggested model, accuracy delta, estimated monthly savings, and status (New/Applied/Dismissed).
- **v3: Cost Intelligence page data:** Aggregates cost-by-model (horizontal bar chart data), cost-by-team (requires resource tags), 30-day cost trend (daily granularity), and active recommendation count.

### 1.5 Discovery Confidence Scoring (New, v3) — maps to Design Spec: Confidence Column

Assigns a confidence score to each discovered workload:

- **Bedrock workloads:** Discovered via typed CloudTrail events (`InvokeModel` with explicit service/model). Confidence = "high" (>95% accuracy).
- **SageMaker managed models:** Discovered via `CreateEndpoint`/`InvokeEndpoint` with known model names. Confidence = "medium" (85-95% accuracy).
- **SageMaker custom endpoints:** Pattern-matched via endpoint naming conventions and invocation patterns. Confidence = "unverified" (<85% accuracy). User can confirm via "Confirm this workload" action.
- **Design mapping:** Confidence badge in workload table (Verified green, Medium yellow, Unverified yellow with warning). Per PRD v2 Failure Scenario 1: if aggregate discovery accuracy <90%, pivot to Bedrock-only and hide SageMaker rows with explanatory Flashbar.

### 1.4 Console Frontend (New Page) — maps to Design Spec: Full Component Mapping (Section: Phase 2)

Single-page React app on Cloudscape v3.x at `console.aws.amazon.com/cloudwatch/home#ai-control-plane`. Full component mapping per design-spec-v3.md Phase 2. v3 additions: Cost Intelligence page (side nav route), cost recommendation Alert component, confidence badge column. Data fetching via React Query (30s table poll, 5 min charts, 15 min cost data). URL-based state for deep-linkable filters/sort/page. L breakpoint primary, M/S/XS degrade gracefully.
...
| `/ai-control-plane/cost/summary` | GET | Cost summary: total spend, trend, optimization potential, recommendation count | 403, 500 |
| `/ai-control-plane/cost/by-model` | GET | Cost aggregated by model with tier classification | 403, 500 |
| `/ai-control-plane/cost/by-team` | GET | Cost aggregated by team (requires resource tags) | 403, 500 |
| `/ai-control-plane/cost/recommendations` | GET | Active recommendations: workload, current/suggested model, accuracy delta, savings | 403, 500 |
| `/ai-control-plane/cost/recommendations/{id}/dismiss` | PUT | Dismiss a recommendation (persists in user preferences) | 404, 403, 500 |
| `/ai-control-plane/cost/trend` | GET | 30-day cost trend with daily granularity | 403, 500 |
...
### v3 Additions: Cost Intelligence and Confidence

[AC-19] Traces to: JTBD-2 / DD-Cost-Intelligence-Page
GIVEN the Cost Intelligence side nav item is clicked
WHEN the Cost Intelligence page renders
THEN the user sees: 4 cost summary cards, cost-by-model horizontal bar chart with tier coloring, cost-by-team horizontal bar chart, recommendations table with sortable savings column, and 30-day cost trend chart

[AC-20] Traces to: JTBD-2 / DD-Cost-Recommendation-Alert
GIVEN a workload with an active cost optimization recommendation
WHEN the user opens the Split Panel and switches to the Cost tab
THEN an info Alert appears at top showing: current model, suggested model, accuracy equivalence percentage, estimated monthly savings, "Show Details" button (navigates to Cost Intelligence page), and "Dismiss" button (hides alert for this workload)

[AC-21] Traces to: PRD-v2-Failure-1 / DD-Confidence-Column
GIVEN the workload table is displayed
WHEN rows render
THEN each row shows a confidence badge (Verified/Medium/Unverified) based on discovery method, the Confidence column is sortable, and SageMaker pattern-matched workloads show "Unverified" with a yellow warning badge

[AC-22] Traces to: DD-Page-Switching
GIVEN the side navigation
WHEN the user clicks a page item (Overview, Cost Intelligence, Maturity, Settings)
THEN the main content area switches to the selected page, the breadcrumb updates, and the side nav active indicator moves to the selected item

[AC-23] Traces to: JTBD-2 / DD-Cost-Recommendation-Dismiss
GIVEN a cost recommendation Alert is visible in the Cost tab
WHEN the user clicks "Dismiss"
THEN the alert hides for that workload, a screen reader announcement confirms dismissal, and the recommendation status changes to "Dismissed" in the Cost Intelligence table
...
| TD-10 | Cost recommendation dismissal is session-only | Prototype: Cost tab Alert | Persist dismissals in DynamoDB user preferences table; load dismissed IDs on page init | P1 | S4 |
| TD-11 | Cost by Team chart empty without tags | Prototype: Cost Intelligence page | Show empty state with "Set up cost attribution" CTA linking to Tag Editor; check tag completeness on first load | P2 | S5 |
| TD-12 | Confidence badge is hardcoded per workload | Prototype: Table confidence column | Connect to Discovery Confidence API; update confidence on each discovery cycle; enable "Confirm workload" action for unverified entries | P1 | S3 |
| TD-13 | Cost Intelligence chart uses DOM bars, not proper charting | Prototype: Cost Intelligence page | Replace horizontal bar charts with Chart.js or D3 for proper axis scaling, tooltips, and click-to-filter interaction | P2 | S4 |
| TD-14 | Model comparison uses static benchmark data | Prototype: Recommendations table | Build benchmark service that tracks accuracy per task-type per model pair from customer telemetry (aggregated, anonymized) | P2 | S6 |
...
| Cost Intelligence Page Usage | Engagement (Counter) | 30% of active users visit weekly by month 3 | RUM page views on `/ai-control-plane/cost` | AI-CP-Frontend RUM dashboard |
| Cost Recommendation Acceptance Rate | Revenue (Rate) | 15% of recommendations acted on within 30 days | Click tracking on "Show Details" + model change events | AI-CP-Cost dashboard |
| Discovery Confidence Accuracy | Quality (Gauge) | < 5% false positive rate for "Verified" badges | "Not an AI workload" feedback on verified entries | AI-CP-Discovery dashboard |
...
|-------|----------|---------|--------|
| 1 — Research | research-v2.md | v2 | Complete (6,450 words, 18 sources, Opportunity-Solution Tree) |
| 2 — PRD | prd-v2.md | v2 | Complete (7,770 words, Solution Lineage table, 3 failure scenarios) |
| 3 — Gandalf | gandalf-evaluation-v2.md | v2 | 10/11 PASS (up from 7/10 in v1) |
| 4 — Design | design-spec-v3.md | v3 | Complete (Cost Intelligence UX, failure scenario responses, confidence badges) |
| 5 — Prototype | prototype-v3.html + prototype-notes-v3.md | v3 | Validated (26/26 checks PASS, 1,947 lines) |
| 6 — Launch Readiness | This document (launch-readiness-v3.md) | v3 | Draft |
| 7 — Post-Launch | — | v0.1.0 | Deferred (triggers 30+ days post-GA) |
