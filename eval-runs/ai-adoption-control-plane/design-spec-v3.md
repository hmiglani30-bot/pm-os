...
**v3 changes from v2:** Updated to align with PRD v2 inputs: (1) Cost Intelligence tab now includes quality-aware model recommendation UX derived from PRD Capability 4, (2) Solution Lineage table from PRD informs capability placement and priority, (3) enhanced persona context (Maya's day-in-the-life, Raj's audit workflow) drives FTUX improvements, (4) Failure Scenario thresholds from PRD v2 (10% misidentification, 8s render, 40% maturity opt-out) are now embedded as design constraints with specific UX responses.
...
**Component Library:** Cloudscape Design System
**Primary User:** Maya Chen -- Sr. Cloud Operations Lead at a mid-size fintech (800 employees, AWS-primary). Manages 4 SREs, 6+ AI deployments across Bedrock, SageMaker, and third-party models. Spends ~5 hours/week manually correlating AI data across 5 consoles. Needs to answer her VP's weekly question: "Are we using AI responsibly, and is it worth what we're spending?"
**Secondary User:** Raj Patel -- CISO at a healthcare SaaS company (2,000 employees, HIPAA-regulated). Relies on quarterly manual audits. Cannot demonstrate AI governance posture to auditors without weeks of manual evidence gathering. 76% of CIOs share his concern about unchecked AI (Logicalis 2026).
**Gandalf Constraints (all passed in v2 evaluation):** Technical feasibility via Security Hub precedent (10/11 pass); 3 falsifiable failure scenarios with UX implications: (a) Discovery accuracy <90% triggers Bedrock-only scope -- design must surface confidence indicators, (b) Dashboard render >8s triggers batch mode -- design must handle stale data gracefully, (c) Maturity opt-out >40% triggers demotion -- design must make maturity non-intrusive and dismissable.
**Pricing Constraint:** v1 ships free as retention play ($100-150M protected revenue estimate). No pricing UX, no paywall, no upsell modals. The console must feel like a natural CloudWatch extension, not an add-on product.
...
| Cost tab | **Bar Chart** + **Key Value Pairs** + **Alert** (info) | Cost by day (bar), total, per-invocation, model optimization recommendation (PRD v2 Capability 4: quality-aware suggestion with estimated savings) |
| Cost Recommendation Card | **Alert** (info) inside Cost tab | "This workflow uses Claude 3.5 Sonnet for classification -- similar workloads show 85% equivalent accuracy on Haiku at 60% lower cost. Estimated savings: $2,400/month." Dismiss + "Show details" link to Cost Intelligence page. |
...
## v3 Additions: PRD v2 Alignment

### Cost Intelligence Recommendation UX (PRD v2 Capability 4)

PRD v2 adds quality-aware cost optimization recommendations. This requires new UX not in v2 design spec.

**In-context recommendation (Split Panel Cost tab):**
An Alert (info) appears at the top of the Cost tab when the system detects an optimization opportunity:

```
[Info icon] Cost Optimization Available
This workflow uses Claude 3.5 Sonnet for boilerplate classification.
Similar workloads show 85% equivalent accuracy on Claude 3 Haiku at 60% lower cost.
Estimated savings: $2,400/month.

[Show Details]    [Dismiss]
```

- "Show Details" navigates to Cost Intelligence side nav page with this workload pre-filtered.
- "Dismiss" hides for this workload, recordable in user preferences. Does not suppress recommendations for other workloads.
- Recommendation appears ONLY when confidence is high (>80% accuracy equivalence from benchmark data). No speculative suggestions.

**Cost Intelligence page (dedicated side nav item):**

| Section | Component | Content |
|---------|-----------|---------|
| Cost summary cards | Cards in 2-col grid | Total AI Spend (month), Spend Trend (vs. last month), Optimization Potential ($), Active Recommendations |
| Cost by model | Horizontal bar chart | All models ranked by spend. Color = model tier (premium/standard/economy). Click bar to filter table below. |
| Cost by team | Horizontal bar chart | Teams ranked by AI spend (requires resource tags). Click to filter. |
| Recommendations table | Table + Status Indicator | Columns: Workload, Current Model, Suggested Model, Accuracy Delta, Monthly Savings, Status (New/Applied/Dismissed). Sortable by savings. |
| Cost trend | Line chart | 30-day AI spend with model breakdown overlay. |

**PRD v2 End-to-End scenario alignment:** Maya's "cost conversation" with her VP maps directly to: open Cost Intelligence page > show cost by model chart > show cost by team chart > show recommendation for code review workflow switching from Sonnet to Haiku. The VP sees a clear picture in under 5 minutes -- matching JTBD #1.

### Failure Scenario Design Responses (PRD v2 Section 3)

Each PRD v2 failure scenario has a specific UX implication:

| Failure Scenario | Threshold | Design Response |
|-----------------|-----------|-----------------|
| Discovery accuracy <90% | Pivot to Bedrock-only | Table adds "Confidence" column showing detection confidence per workload. Low-confidence rows (SageMaker pattern-matched) show yellow "Unverified" badge with "Confirm this workload" action. If pivot triggers, SageMaker rows are hidden and a Flashbar explains: "Discovery is now limited to Bedrock workloads for accuracy. SageMaker discovery will return in a future update." |
| Dashboard render >8s at 50 accounts | Switch to batch aggregation | Design already handles this via stale data state (timestamp + Refresh button). If pivot triggers, the Date Range Picker default changes from "Last 7 days" to "Last 24 hours (cached)" with a note: "Data refreshes every 15 minutes. Click Refresh for latest." Skeleton screens remain during refresh. |
| Maturity opt-out >40% | Demote to optional | Maturity section moves from default-visible below-fold to Settings > "Show maturity score" toggle (off by default). The below-fold space is replaced with a simpler "Governance Coverage" bar: single horizontal progress bar showing "68% of workloads have guardrails" with a "View details" link. No prescriptive levels, no recommended actions in the main view. |

### Solution Lineage Design Traceability

Each design decision traces to a specific PRD v2 Solution Lineage entry:

| Design Element | PRD Capability | Opportunity-Direction | Design Rationale |
|---------------|---------------|----------------------|-----------------|
| CloudWatch side nav integration | AI Asset Discovery | Opp 1, Dir A (CloudWatch-embedded) | Zero new console; existing muscle memory; 500K+ user base gets zero-friction access |
| Summary cards (4 KPIs) | Unified AI Dashboard | Opp 1, Dir A | 10-second health check pattern (Datadog); Direction B (standalone service) rejected because it requires new navigation context |
| Split Panel Guardrails tab | Guardrail Monitoring | Opp 1, Dir A | Contextual surfacing (Salesforce pattern); guardrail status shown in workload context, not a separate page |
| Cost tab + Cost Intelligence page | Cost Intelligence | Opp 3, Dir A (model comparison) | Quality-aware recommendations differentiate from Datadog/ServiceNow cost tracking (Direction B: anomaly-only was rejected as undifferentiated) |
| Maturity section below-fold | Adoption Maturity Score | Opp 5, Dir B (maturity model) | Below-fold placement respects weekly cadence of strategic metric; Direction A (Config rules) rejected as too narrow for maturity assessment |
...
3. **Cost tab recommendation Alert** -- info Alert with "Show Details" and "Dismiss" actions. NEW in v3.
4. **Property Filter** -- at least 3 filterable properties (service type, health status, guardrail status) with token-based filtering.
5. **Date Range Picker** -- global, updates all visible data.
6. **Summary cards** with realistic data: Total Workloads (14), Guardrail Coverage (68%), Unhealthy (2), Monthly Cost ($12,400).
7. **Status Indicators** in table cells (green/yellow/red with icons, not color alone).
8. **Skeleton loading states** on initial load (not spinners).
9. **Keyboard nav**: `/` to focus filter, `Esc` to close panel, arrow keys in table.
10. **Cost Intelligence page** -- side nav item with cost-by-model bar chart, cost-by-team chart, recommendations table. NEW in v3.
...
### v3-Specific Feedback (from PRD v2 alignment)

5. **Cost recommendation confidence threshold:** The Cost tab recommendation Alert needs a confidence threshold. Design assumes >80% accuracy equivalence from benchmark data, but the PRD should define this threshold and the data source for benchmarks. Without real benchmark data, recommendations risk eroding trust.
6. **Raj's audit workflow needs export customization:** PRD v2 describes Raj generating a compliance summary in 5 minutes. The current design supports PDF/CSV export from an Actions dropdown, but Raj likely needs: (a) scope selection (which accounts, which date range, which compliance framework), (b) branded output for board presentations, (c) scheduled recurring reports. PRD should specify v1 vs. v2 report capabilities.
7. **Cost by team requires tagging convention:** The Cost Intelligence page shows cost by team, but this depends on consistent resource tagging. If customers don't tag AI resources by team, this chart is empty. The FTUX should include a "Set up cost attribution" step that checks for tag completeness and guides users to Tag Editor. PRD should add tagging guidance to the onboarding requirements.

---

## v3 Changelog

| Change | Section | What Changed | Why |
|--------|---------|-------------|-----|
| Frontmatter | Header | prd-version: v2 (was v1+gandalf-fixes) | Inputs changed |
| Executive Summary | Header | Added v3 change summary paragraph | Transparency on what's new |
| Design Context | Header | Enhanced persona details from PRD v2 (Maya's day-in-the-life, Raj's audit workflow, company sizes, tools used). Added pricing constraint (free v1). Added failure scenario design constraints. | Richer persona context drives better UX decisions |
| Cost Recommendation Card | Cloudscape Mapping | New component: Alert (info) inside Cost tab with model comparison recommendation | PRD v2 Capability 4 (quality-aware cost optimization) |
| Cost Intelligence page | v3 Additions | Full page design: cost summary cards, cost-by-model chart, cost-by-team chart, recommendations table | PRD v2 end-to-end scenario: Maya answers VP's cost question in 5 minutes |
| Failure scenario responses | v3 Additions | Design-level responses to all 3 PRD v2 failure scenarios | Each pivot trigger now has a specific UX fallback |
| Solution lineage traceability | v3 Additions | Each design element traced to PRD v2 Solution Lineage and opportunity tree | Maintains research-to-design traceability introduced in v0.3.0 upgrade |
| Handoff notes | Prototype Builder | Added items 3 (cost recommendation) and 10 (Cost Intelligence page) | New interactions for prototype to implement |
| PRD feedback | Feedback section | Added 3 new feedback items (cost confidence, Raj's export, tagging) | PRD v2 introduced new capabilities that surface new ambiguities |
